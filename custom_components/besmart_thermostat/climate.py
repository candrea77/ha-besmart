...
version: 0.5 (DataUpdateCoordinator + entity fixes)
"""
import logging
...
async def async_setup_entry(
    ...
    new_entities = []
    # PATCH 0.5: topology now lives on the coordinator (no custom attributes
    # on ConfigEntry).
...
class Thermostat(CoordinatorEntity[BesmartDataUpdateCoordinator], ClimateEntity):
    ...
    def __init__(self, coordinator, config_entry, wifi_box, room_id, room_name, device_info):
        ...
        # name = <roomName> Thermostat (entity_id is generated by HA from
        # device + name thanks to _attr_has_entity_name).
        # PATCH 0.5: removed the dead async_generate_entity_id call (it wrote
        # to self._entity_id, which HA never reads).
    ...
    def _update_attrs(self) -> None:
        ...
        # Safe casting for all dictionary reads
        try:
        ...
        # Current mode
        try:
            self._current_state = int(thermostat.get("mode", self.IDLE))
        except (ValueError, TypeError):
            # PATCH 0.5: fall back to IDLE (4), not AUTO (0), consistently
        ...
        if self._current_state == self.AUTO:
            ...
            try:
                # PATCH 0.5: use HA's timezone-aware clock instead of
                # datetime.today() (the container timezone may differ from the
                # timezone configured in Home Assistant, shifting the program
                # slot by hours).
                now = dt_util.now()
                ...
                _LOGGER.warning("Error parsing thermostat program: %s", ex)
                self._tempSetMark = "2"

            # Extract manual toggle for ECO (night) mode
            try:
                # advance option is used for switching to the ECO mode
                # (automatically disables at holiday_end_time)
                if thermostat.get("advance") == "1":
                    self._holiday_end_time = int(thermostat.get("holiday_end_time", 0))
                    self._tempSetMark = "1"
            except Exception as ex:
                _LOGGER.warning("Error parsing advance/holiday_end_time: %s", ex)
                self._holiday_end_time = None
    ...
    async def async_set_hvac_mode(self, hvac_mode):
        ...
        if hvac_mode == HVACMode.OFF:
            ...
            if season is not None and self._season != season:
                # PATCH 0.5: surface command failures in the UI instead of
                # silently ignoring them.
                ok = await self._cl.setThermostatSeason(self._wifi_box, self._room_id, season)
                if not ok:
                    raise HomeAssistantError(
                        f"Failed to set season for {self._room_name} on BeSMART cloud"
                    )
    ...
    async def async_set_preset_mode(self, preset_mode):
        ...
        ok = await self._cl.setThermostatMode(self._wifi_box, self._room_id, mode)
        if not ok:
            raise HomeAssistantError(
                f"Failed to set preset {preset_mode} for {self._room_name} on BeSMART cloud"
            )
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        _LOGGER.debug(
            "setting new temp %s %s %s", self._tempSetMark, self._room_name, temperature
        )
        # The mark itself selects which set-point is written.
        ok = await self._cl.setThermostatTemp(
            self._wifi_box, self._room_id, temperature, self._tempSetMark
        )
        if not ok:
            raise HomeAssistantError(
                f"Failed to set temperature for {self._room_name} on BeSMART cloud"
            )
        await self.coordinator.async_request_refresh()
